...
# Knowledge notes marker: [KNOWLEDGE:title | content]
# Title and content are separated by the first " | " (pipe with spaces)
KNOWLEDGE_PATTERN = re.compile(r"\[KNOWLEDGE:\s*([^\]|]+?)\s*\|\s*(.*?)\]", re.IGNORECASE | re.DOTALL)
...
class KnowledgeEntry:
    """A knowledge note write request [KNOWLEDGE: title | content]."""
...
def extract_memory_markers(
    ...
    """Extract [JOURNAL:], [MEMORY_SEARCH:], [WIKIPEDIA:], [WEB_SEARCH:], HA, and [KNOWLEDGE:] markers.
    ...
    Returns a 10-tuple:
        ...
        (9) knowledge_entries: [KNOWLEDGE: title | content] to write to knowledge notes
    """
    ...
    def repl_knowledge(m: re.Match) -> str:
        title = m.group(1).strip()
        content = m.group(2).strip()
        if title:
            knowledge_entries.append(KnowledgeEntry(title=title, content=content))
        return ""
    ...
    cleaned = KNOWLEDGE_PATTERN.sub(repl_knowledge, cleaned)
    return (
        cleaned.strip(),
        journals,
        searches,
